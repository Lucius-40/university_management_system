...
const table_query = `
    ...
    CREATE TABLE IF NOT EXISTS terms (
        ...
        ALTER TABLE terms ADD COLUMN IF NOT EXISTS max_credit DECIMAL(4, 1) NOT NULL DEFAULT 23.0;
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1
            FROM pg_constraint
            WHERE conname = 'terms_max_credit_positive_chk'
        ) THEN
            ALTER TABLE terms
...
CREATE TABLE IF NOT EXISTS current_state (
    ...
    -- Enforce singleton pattern for the university-wide current state row.
    DO $$
    DECLARE
        v_keep_ctid tid;
    BEGIN
    ...
    ALTER TABLE current_state ALTER COLUMN id SET DEFAULT 1;
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1
            FROM pg_constraint
            WHERE conname = 'current_state_singleton_pk'
        ) THEN
            BEGIN
                ALTER TABLE current_state
                ADD CONSTRAINT current_state_singleton_pk PRIMARY KEY (id);
            EXCEPTION
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1
            FROM pg_constraint
            WHERE conname = 'current_state_singleton_chk'
        ) THEN
            ALTER TABLE current_state
            ADD CONSTRAINT current_state_singleton_chk CHECK (id = 1);
        END IF;
    END $$;
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1
            FROM pg_constraint
            WHERE conname = 'current_state_reg_window_chk'
        ) THEN
            ALTER TABLE current_state
            ADD CONSTRAINT current_state_reg_window_chk
    ...
    BEGIN
        IF NOT EXISTS (
            SELECT 1
            FROM pg_constraint
            WHERE conname = 'current_state_term_window_chk'
        ) THEN
            ALTER TABLE current_state
            ADD CONSTRAINT current_state_term_window_chk
