...
class TableModel {
    ...
    async createFunctions() {
        const sql = `
            ...
            AS $$
            BEGIN
                IF NEW.term_start IS NOT NULL
                   AND NEW.term_end IS NOT NULL
                   AND (
                       TG_OP = 'INSERT'
                       OR
                       NEW.term_start IS DISTINCT FROM OLD.term_start
                       OR NEW.term_end IS DISTINCT FROM OLD.term_end
                   ) THEN
                    UPDATE terms
                    SET start_date = NEW.term_start,
                        end_date = NEW.term_end;
                END IF;

                IF NEW.term_start IS NOT NULL THEN
