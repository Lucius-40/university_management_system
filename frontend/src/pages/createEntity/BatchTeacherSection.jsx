...
const BatchTeacherSection = ({ activeTab, activeMode, loading, result, onSubmit }) => {
  ...
  if (activeTab !== "teacher" || activeMode !== "batch") return null;
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold">Batch Import Teachers</h2>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {previewRows.map((row, index) => (
                  <tr key={`${row["Personal Email"] || index}`} className="border-t">
                    <td className="px-3 py-2">{row["Full Name"]}</td>
                    <td className="px-3 py-2">{row["Personal Email"]}</td>
                    <td className="px-3 py-2">{row["Appointment Type"]}</td>
          ...
          {Array.isArray(result.results) && result.results.some((item) => item.status === "failed") && (
            <div className="max-h-48 overflow-y-auto rounded border border-red-200 bg-red-50 p-2 text-xs text-red-700">
              {result.results
                ...
                .map((item) => (
                  <div key={`teacher-failed-${item.row}`}>
...
export default BatchTeacherSection;
